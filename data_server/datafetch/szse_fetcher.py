...
class SZSEFetcher:
    ...
    async def get_fund_list(self, date_str: Optional[str] = None) -> Optional[pd.DataFrame]:
        """
        获取深交所基金列表
        CATALOGID=1105
        ...
        """
        logger.info(f"获取深交所基金列表，日期: {date_str}")
        return await self._fetch_xlsx_data("1105")
    
    async def get_convertible_bond_list(self, date_str: Optional[str] = None) -> Optional[pd.DataFrame]:
        """
        获取深交所可转换债券列表
        CATALOGID=1277
    ...
    async def get_all_stocks(self) -> pd.DataFrame:
        ...
        # 合并数据
        big_df = pd.DataFrame()
        for i, symbol in enumerate(["A股列表", "B股列表"]):
            df = results[i]
