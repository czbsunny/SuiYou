...
const fetchAssetData = async () => {
  try {
    ...
    asset.value = {
      id: assetId,
      name: '微信钱包',
      totalBalance: 6666.66,
      availableBalance: 6666.66,
      frozenBalance: 0,
      currency: 'CNY',
...
const fetchAccountData = async () => {
  try {
    ...
    account.value = {
      institution: 'WECHAT',
      institutionIdentifier: 'wxid_1234567890'
    };
  } catch (error) {
...
const attr = computed(() => {
  try {
    return JSON.parse(asset.value?.attributes || '{}');
  } catch (e) {
    return {};
  }
});
...
const walletThemeClass = computed(() => {
  const inst = account.value?.institution?.toUpperCase();
...
const handleSeeMore = () => uni.navigateTo({ url: `/pages/assets/history?id=${asset.value.id}` });
</script>
